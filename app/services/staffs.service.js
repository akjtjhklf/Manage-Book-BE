...
class StarffsService {
    ...
    async signUp(userData) {
        try {
            ...
            if (userData.password=='') {
                return new ApiError(401, 'Mật khẩu không thể trống');
            }
            if (userData.fullName=='') {
                return new ApiError(401, 'Họ và tên không thể trống');
            }
            if (userData.username=='') {
                return new ApiError(401, 'Tên người dùng không thể trống');
            }
            if (userData.position=='') {
                return new ApiError(401, 'Chức vụ không thể trống');
            }
            if (userData.address=='') {
                return new ApiError(401, 'Địa chỉ không thể trống');
            }
            if (userData.phone=='') {
                return new ApiError(401, 'Số điện thoại không thể trống');
            }
            // Hash mật khẩu trước khi lưu vào CSDL
            // Thêm mật khẩu đã được hash vào dữ liệu người dùng
            if (user) {
                return new ApiError(401, 'Tên người dùng đã tồn tại');
            }
            ...
            return { newUser: newUser, message: "Đăng ký thành công!", isSuccess: true };
        } catch (error) {
    ...
    async signIn(username, password) {
        try {
            ...
            if (!user) {
                return new ApiError(401, 'Tên người dùng không tồn tại');
            }

            // So sánh mật khẩu đã nhập với mật khẩu đã lưu của người dùng
            if (password != user.password) {
                return new ApiError(401, 'Sai mật khẩu');
            }

            // Tạo một JWT token cho người dùng
            const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, { expiresIn: '1h' });
            console.log(token)
            return { token, message: "Đăng nhập thành công!", isSuccess: true };
        } catch (error) {
