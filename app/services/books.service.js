...
class BooksService {
    ...
    async create(data) {
        if (data.name=='') {
            return new ApiError(400, "Tên không được để trống");
        }
        const book = this.extractBookData(data);
        const result = await this.Books.insertOne(
            book
        );
        return { result: result.value, message: "Tạo thành công!", isSuccess: true };
    }
    async find(filter) {
    ...
    async update(id, payload) {
        ...
        if (!result) {
            return new ApiError(404, "Không tìm thấy sách!");
    ...
    async delete(id) {
        ...
        if (!result) {
            return next(
                new ApiError(404, "Không tìm thấy sách")
            );
        }
        return { result,message: "Xóa sách thành công",isSuccess:true }
    }
    async findFavorite() {
